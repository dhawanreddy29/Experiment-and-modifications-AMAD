...
def wasserstein_loss(pred, target):
    """
    Calculate the Wasserstein loss between two distributions using Earth Mover's Distance.
    Args:
    ...
    """
    pred_cumsum = torch.cumsum(pred.sort()[0], dim=0)
    target_cumsum = torch.cumsum(target.sort()[0], dim=0)
    return torch.mean(torch.abs(pred_cumsum - target_cumsum))


def calculate_bg_spp_loss(logps, mask, boundaries, normalizer=10, weights=None):
    """
    Calculate boundary-guided semi-push-pull contrastive loss using Wasserstein loss.
    Args:
        logps: log-likelihoods, shape (N, )
        mask: 0 for normal, 1 for abnormal, shape (N, )
    ...
    # Calculate Wasserstein loss for normal samples
    wasserstein_loss_n = wasserstein_loss(normal_logps_inter, torch.zeros_like(normal_logps_inter))

    b_a = boundaries[1]
    anomaly_logps = logps[mask == 1]    
    anomaly_logps_inter = anomaly_logps[anomaly_logps >= b_a]

    # Calculate Wasserstein loss for abnormal samples
    wasserstein_loss_a = wasserstein_loss(anomaly_logps_inter, torch.ones_like(anomaly_logps_inter))

    if weights is not None:
        nor_weights = weights[mask == 0][normal_logps <= b_n]
        wasserstein_loss_n = wasserstein_loss_n * nor_weights
        ano_weights = weights[mask == 1][anomaly_logps >= b_a]
        wasserstein_loss_a = wasserstein_loss_a * ano_weights
    
    wasserstein_loss_n = torch.mean(wasserstein_loss_n)
    wasserstein_loss_a = torch.mean(wasserstein_loss_a)

    return wasserstein_loss_n, wasserstein_loss_a


def calculate_bg_spp_loss_normal(logps, mask, boundaries, normalizer=10, weights=None):
    """
    Calculate boundary-guided semi-push-pull contrastive loss for normal samples using Wasserstein loss.
    Args:
        logps: log-likelihoods, shape (N, )
        mask: 0 for normal, 1 for abnormal, shape (N, )
        boundaries: normal boundary and abnormal boundary
