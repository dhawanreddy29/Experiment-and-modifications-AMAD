...
def calculate_bg_spp_loss(logps, mask, boundaries, normalizer=10, weights=None):
    """
    Calculate boundary guided semi-push-pull contrastive loss.
    Args:
        logps: log-likelihoods, shape (N, )
...
def cross_entropy_loss(logps, mask, normalizer=10):
    logps = logps / normalizer
    normal_logps = logps[mask == 0]
    abnormal_logps = logps[mask == 1]

    # Creating target distribution for Cross-Entropy
    target_distribution = torch.distributions.Normal(0, 1).log_prob(torch.linspace(-3, 3, steps=logps.size(0)))
    
    # Calculate cross-entropy loss for normal and abnormal logps
    ce_loss_normal = F.binary_cross_entropy_with_logits(normal_logps, target_distribution)
    ce_loss_abnormal = F.binary_cross_entropy_with_logits(abnormal_logps, target_distribution)

    return ce_loss_normal + ce_loss_abnormal
