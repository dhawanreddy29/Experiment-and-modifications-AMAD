...
def kl_divergence_loss(p_log_probs, q_log_probs):
    """
    Calculate the KL divergence between two distributions.
    Args:
        p_log_probs: Tensor of the data distribution log-probabilities.
        q_log_probs: Tensor of the learned distribution log-probabilities.
    Returns:
        kl_loss: KL divergence loss.
    """
    kl_loss = torch.mean(p_log_probs - q_log_probs)
    return kl_loss


def calculate_bg_spp_loss(logps, mask, boundaries, normalizer=10, weights=None):
    """
    Calculate boundary-guided semi-push-pull contrastive loss using KL divergence.
    Args:
        logps: log-likelihoods, shape (N, )
        mask: 0 for normal, 1 for abnormal, shape (N, )
    ...
    # Calculate KL divergence loss for normal samples
    kl_loss_n = kl_divergence_loss(normal_logps_inter, logps[mask == 0][normal_logps <= b_n])

    b_a = boundaries[1]
    anomaly_logps = logps[mask == 1]    
    anomaly_logps_inter = anomaly_logps[anomaly_logps >= b_a]

    # Calculate KL divergence loss for abnormal samples
    kl_loss_a = kl_divergence_loss(anomaly_logps_inter, logps[mask == 1][anomaly_logps >= b_a])

    if weights is not None:
        nor_weights = weights[mask == 0][normal_logps <= b_n]
        kl_loss_n = kl_loss_n * nor_weights
        ano_weights = weights[mask == 1][anomaly_logps >= b_a]
        kl_loss_a = kl_loss_a * ano_weights
    
    kl_loss_n = torch.mean(kl_loss_n)
    kl_loss_a = torch.mean(kl_loss_a)

    return kl_loss_n, kl_loss_a


def calculate_bg_spp_loss_normal(logps, mask, boundaries, normalizer=10, weights=None):
    """
    Calculate boundary-guided semi-push-pull contrastive loss for normal samples using KL divergence.
    Args:
        logps: log-likelihoods, shape (N, )
        mask: 0 for normal, 1 for abnormal, shape (N, )
        boundaries: normal boundary and abnormal boundary
